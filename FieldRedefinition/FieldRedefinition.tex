...
Therefore one can view the negative of the torsion of the basic connection as a possible choice for $\zeta$ along the foliation of the anchor; however, this may not always be possible because one has to lift the anchor. For example, if $\rho \equiv 0$, then $0 = \zeta \circ (\rho, \rho) = - t_{\nabla^{\mathrm{bas}}} = t_{\nabla_\rho} = [\cdot, \cdot]_E$ would imply an abelian structure on $E$. 
In fact, if we can choose $\zeta \in \Omega^2(N;E)$ such that $\zeta \circ (\rho, \rho) = - t_{\nabla^{\mathrm{bas}}}$, we get:
